...
def product_detail(request, product_id):
    ...
    if request.user.is_authenticated:
        user_wishlist = Wishlist.objects.filter(
            user=request.user, products=product
        ).exists()
...
def edit_product(request, product_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request, "Sorry, only store owners can edit products."
        )
        return redirect(reverse("index"))
...
def add_review(request, product_id):
    ...
    return render(
        ...
        {"product": product, "form": ReviewForm()}
    )
...
def edit_review(request, review_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Failed to update the review. Ensure the form is valid."
            )
    else:
